...
let birds =
  ...
  let fly =
    ...
    Cmd.v info Term.(const (fun _ _ -> ()) $ bird $ speed)
  in
  let land' =
    let info = Cmd.info "land" ~doc:"Land birds." in
    Cmd.v info Term.(const (fun _ -> ()) $ bird)
  in
  let info = Cmd.info "birds" ~doc:"Operate on birds." in
  Cmd.group ~default:Term.(const (fun _ -> ()) $ kind) info [fly; land']
...
let fishs =
  ...
  Cmd.v info Term.(const (fun _ -> ()) $ name')
...
let camels =
  ...
  Cmd.v info Term.(const (fun _ _ -> ()) $ bactrian $ herd)
